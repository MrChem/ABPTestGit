...
using WENTE.Migrations.SeedData;
...
namespace WENTE.Tests
{
    public abstract class WENTETestBase : AbpIntegratedTestBase<WENTETestModule>
    {
        ...
        protected void UsingDbContext(Action<WENTEDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<WENTEDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<WENTEDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<WENTEDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<WENTEDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WENTEDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<WENTEDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WENTEDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<WENTEDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<WENTEDbContext, Task<T>> func)
        {
            T result;
